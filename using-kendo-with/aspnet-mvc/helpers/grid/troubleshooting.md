...
By default Kendo Grid for ASP.NET MVC should make POST requests when configured for [ajax binding](/using-kendo-with/aspnet-mvc/helpers/grid/ajax-binding).
...
Kendo JavaScript files. More info can be found in the [introduction help topic](/using-kendo-with/aspnet-mvc/introduction).
...
More info can be found in the [troubleshooting help topic](/using-kendo-with/aspnet-mvc/troubleshooting).
...
[ajax binding](/using-kendo-with/aspnet-mvc/helpers/grid/ajax-binding) help topic for further instructions.
...
Check the ["How do I avoid circular reference exceptions"](/using-kendo-with/aspnet-mvc/helpers/grid/faq#how-do-i-avoid-circular-reference-exceptions?) FAQ section for further information.
...
2. [Use a View Model](/using-kendo-with/aspnet-mvc/helpers/grid/faq#how-do-i-convert-my-models-to-view-model-objects?) to serialize only the required properties of your model.
...
This will happen if the server template is set but the grid is configured for ajax binding. Set the [ClientTemplate](/using-kendo-with/aspnet-mvc/helpers/grid/configuration#clienttemplate) as well.
This will also happen if only the client template is set but the grid is configured for server binding. Set the [Template](/using-kendo-with/aspnet-mvc/helpers/grid/configuration#template) as well.
...
Check the [server binding](/using-kendo-with/aspnet-mvc/helpers/grid/server-binding#client-side-events-and-server-binding) help topic for further details.
