...
meta_title: How to bind Kendo DropDownList for PHP to remote service which returns JSON
meta_description: Learn how to bind Kendo UI DropDownList for PHP to JSON
slug: php-dropdownlist-remote-binding
publish: true
relatedDocs: php-ui-dropdownlist, php-dropdownlist-overview
---
# Remote Binding

This help topic shows how to bind Kendo DropDownList for PHP to JSON response. Remote binding means that filter operation
will happen on the server-side.

## Binding to array returned by PDO

### Configure DropDownList for Remote Binding
...
1. Create a data source and configure it:
        ...
        $dataSource->transport($transport)
                   ->schema($schema);
        ?>
1. Create a dropdownlist, configure its dataTextField option and set its data source.
...
First we will configure a Kendo DropDownList for PHP binding and then we will implement the remote service which will return JSON.

### Configure DropDownList for Remote Binding (using DataSourceResult)
